...
public class SwipeDismissTouchListener extends SwipeTouchListener {
    ...
    public SwipeDismissTouchListener(final boolean enableRightSwipe,
                                     @NonNull final ListViewWrapper listViewWrapper, @NonNull final OnDismissCallback callback) {
        super(enableRightSwipe, listViewWrapper);
        mCallback = callback;
    ...
    protected void afterCancelSwipe(@NonNull final View view, final int position) {
        //TODO my changes, no callback is called so direction of swipe does not matter
    ...
    protected void performDismiss(@NonNull final View view, final int position) {
        ...
        if (enableRightSwipe && swipingRight){
            animator = ValueAnimator.ofInt(view.getHeight(), view.getHeight() - 1).setDuration(mDismissAnimationTime);
        } else {
            animator = ValueAnimator.ofInt(view.getHeight(), 1).setDuration(mDismissAnimationTime);
        }
     ...
     */
    protected void finalizeDismiss(@NonNull final boolean toTheRight) {
        if (mActiveDismissCount == 0 && getActiveSwipeCount() == 0) {
            restoreViewPresentations(mDismissedViews);
            notifyCallback(mDismissedPositions, toTheRight);
     ...
     */
    protected void notifyCallback(@NonNull final List<Integer> dismissedPositions, @NonNull final boolean toTheRight) {
        if (!dismissedPositions.isEmpty()) {
            ...
            mCallback.onDismiss(getListViewWrapper().getListView(), dismissPositions, toTheRight);
        }
    }
